...
public class OffreService implements IService<Offre> {
    ...
    public void update(Offre offre, int id) {
        String sql = "UPDATE Offre SET titre = ?, recrutement_id = ? WHERE id = ?";
        try (PreparedStatement pst = connexion.prepareStatement(sql)) {
            pst.setString(1, offre.getTitre());
            pst.setInt(2, offre.getRecrutement().getId());
            pst.setInt(3, id);
        ...
        try (PreparedStatement pst = connexion.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                return new Offre(
                        rs.getInt("id"),
                        rs.getString("titre"),
                        new RecrutementService().readById(rs.getInt("recrutement_id")) // Fetch full Recrutement separately
                );
            }
        } catch (SQLException e) {
            e.printStackTrace();
