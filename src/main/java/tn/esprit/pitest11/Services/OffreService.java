...
public class OffreService implements IService<Offre> {
    ...
    public void add(Offre offre) {
        String sql = "INSERT INTO Offre (titre, description) VALUES (?, ?)";
    ...
    // UPDATE OFFRE (Including description)
    @Override
    public void update(Offre offre, int id) {
        String sql = "UPDATE Offre SET titre = ?, description = ? WHERE id = ?";
        try (PreparedStatement pst = connexion.prepareStatement(sql)) {
            pst.setString(1, offre.getTitre());
            pst.setString(2, offre.getDescription());
            pst.setInt(3, id);
    ...
    // FETCH DESCRIPTION OF AN OFFRE BY ID
    public String getOffreDescription(int id) {
        String sql = "SELECT description FROM offre WHERE id = ?";
        try (PreparedStatement pst = connexion.prepareStatement(sql)) {
            pst.setInt(1, id);
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
    ...
    // UPDATE DESCRIPTION OF AN OFFRE
    public void updateDescription(int id, String newDescription) {
        String sql = "UPDATE Offre SET description = ? WHERE id = ?";
        try (PreparedStatement pst = connexion.prepareStatement(sql)) {
            pst.setString(1, newDescription);
            pst.setInt(2, id);
            int rowsAffected = pst.executeUpdate();
            if (rowsAffected > 0) {
                System.out.println("Description updated successfully for ID: " + id);
            } else {
                System.out.println("No offre found with ID: " + id);
            }
        } catch (SQLException e) {
