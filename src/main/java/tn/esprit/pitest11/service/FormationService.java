...
import tn.esprit.pitest11.example.DBConnection;
...
public class FormationService implements IService<formations> {
    ...
    public List<String> getAllFormationNames() {
        ...
        String query = "SELECT name FROM formations"; // Assure-toi que ta table s'appelle bien 'formations'

        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery(query)) {

            while (rs.next()) {
                formationNames.add(rs.getString("name"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // Récupérer l'ID d'une formation par son nom
    public int getFormationIdByName(String nomFormation) {
        String query = "SELECT id FROM formations WHERE name = ?";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setString(1, nomFormation);
